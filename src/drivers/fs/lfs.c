 ...
 */
#include <drivers/fs/lfs.h>
#include <drivers/fs/lfs_util.h>
#include <pb/assert.h>
#include <pb/pb.h>
...
static int lfs_bd_flush(lfs_t *lfs, lfs_cache_t *pcache, lfs_cache_t *rcache, bool validate)
{
    if (pcache->block != LFS_BLOCK_NULL && pcache->block != LFS_BLOCK_INLINE) {
        ...
        if (validate) {
            ...
            int res = lfs_bd_cmp(
                lfs, NULL, rcache, diff, pcache->block, pcache->off, pcache->buffer, diff);
            if (res < 0) {
                return res;
            }
...
static lfs_ssize_t
lfs_file_rawwrite(lfs_t *lfs, lfs_file_t *file, const void *buffer, lfs_size_t size);
                                   ...
                                   lfs_size_t gsize)
{
    ...
    while (off >= sizeof(lfs_tag_t) + lfs_tag_dsize(ntag)) {
        ...
        lfs_tag_t tag = ntag;
        int err = lfs_bd_read(
            lfs, NULL, &lfs->rcache, sizeof(ntag), dir->pair[0], off, &ntag, sizeof(ntag));
        if (err) {
            return err;
        }
                            ...
                            void *data)
{
    ...
    while (true) {
        ...
        if (off + lfs_tag_dsize(ptag) < dir->off) {
            ...
            int err = lfs_bd_read(
                lfs, NULL, &lfs->rcache, sizeof(tag), dir->pair[0], off, &tag, sizeof(tag));
            if (err) {
                return err;
            }
        ...
        if (lfs_tag_id(tmask) != 0) {
            ...
            int filter = lfs_dir_traverse(
                lfs, dir, off, ptag, attrs, attrcount, 0, 0, 0, 0, 0, lfs_dir_traverse_filter, &tag);
            if (filter < 0) {
                return filter;
            }
                                     ...
                                     void *data)
{
    ...
    for (int i = 0; i < 2; i++) {
        int err = lfs_bd_read(
            lfs, NULL, &lfs->rcache, sizeof(revs[i]), pair[i], 0, &revs[i], sizeof(revs[i]));
    ...
    for (int i = 0; i < 2; i++) {
        ...
        while (true) {
            ...
            off += lfs_tag_dsize(ptag);
            int err = lfs_bd_read(
                lfs, NULL, &lfs->rcache, lfs->cfg->block_size, dir->pair[0], off, &tag, sizeof(tag));
            if (err) {
            ...
            for (lfs_off_t j = sizeof(tag); j < lfs_tag_dsize(tag); j++) {
                uint8_t dat;
                err = lfs_bd_read(
                    lfs, NULL, &lfs->rcache, lfs->cfg->block_size, dir->pair[0], off + j, &dat, 1);
                if (err) {
                    if (err == LFS_ERR_CORRUPT) {
                        dir->erased = false;
                        break;
...
static int lfs_dir_getgstate(lfs_t *lfs, const lfs_mdir_t *dir, lfs_gstate_t *gstate)
{
    ...
    lfs_stag_t res = lfs_dir_get(
        lfs, dir, LFS_MKTAG(0x7ff, 0, 0), LFS_MKTAG(LFS_TYPE_MOVESTATE, 0, sizeof(temp)), &temp);
...
static int lfs_dir_getinfo(lfs_t *lfs, lfs_mdir_t *dir, uint16_t id, struct lfs_info *info)
{
    ...
    struct lfs_ctz ctz;
    tag = lfs_dir_get(
        lfs, dir, LFS_MKTAG(0x700, 0x3ff, 0), LFS_MKTAG(LFS_TYPE_STRUCT, id, sizeof(ctz)), &ctz);
    if (tag < 0) {
        return (int)tag;
    }
...
static int lfs_dir_commitcrc(lfs_t *lfs, struct lfs_commit *commit)
{
    ...
    while (commit->off < end) {
        ...
        int err = lfs_bd_read(
            lfs, NULL, &lfs->rcache, sizeof(tag), commit->block, noff, &tag, sizeof(tag));
        if (err && err != LFS_ERR_CORRUPT) {
            return err;
        }
    ...
    while (off < end) {
        ...
        for (lfs_off_t i = off; i < noff + sizeof(uint32_t); i++) {
            ...
            uint8_t dat;
            err = lfs_bd_read(
                lfs, NULL, &lfs->rcache, noff + sizeof(uint32_t) - i, commit->block, i, &dat, 1);
            if (err) {
                return err;
            }
...
static int lfs_dir_alloc(lfs_t *lfs, lfs_mdir_t *dir)
{
    ...
    int err = lfs_bd_read(
        lfs, NULL, &lfs->rcache, sizeof(dir->rev), dir->pair[0], 0, &dir->rev, sizeof(dir->rev));
    dir->rev = lfs_fromle32(dir->rev);
    if (err && err != LFS_ERR_CORRUPT) {
        return err;
    }
    ...
    while (true) {
...

        {
            ...
            for (lfs_off_t i = 0; i < skips; i++) {
                ...
                if (i != skips - 1) {
                    err = lfs_bd_read(
                        lfs, NULL, rcache, sizeof(nhead), nhead, 4 * i, &nhead, sizeof(nhead));
                    nhead = lfs_fromle32(nhead);
                    if (err) {
                        return err;
                    }
                            ...
                            void *data)
{
    ...
    while (true) {
        ...
        int count = 2 - (index & 1);
        err = lfs_bd_read(
            lfs, pcache, rcache, count * sizeof(head), head, 0, &heads, count * sizeof(head));
    ...
    while (true) {
        ...
        for (lfs_off_t i = 0; i < file->off; i++) {
            ...
            if (file->flags & LFS_F_INLINE) {
                ...
                err = lfs_bd_read(
                    lfs, &file->cache, &lfs->rcache, file->off - i, file->block, i, &data, 1);
                if (err) {
                    return err;
                }
            }
...
static lfs_ssize_t lfs_file_rawread(lfs_t *lfs, lfs_file_t *file, void *buffer, lfs_size_t size)
{
    ...
    while (nsize > 0) {
        ...
        if (file->flags & LFS_F_INLINE) {
            ...
            int err = lfs_bd_read(
                lfs, NULL, &file->cache, lfs->cfg->block_size, file->block, file->off, data, diff);
            if (err) {
                return err;
            }
        }
...
lfs_file_rawwrite(lfs_t *lfs, lfs_file_t *file, const void *buffer, lfs_size_t size)
{
    ...
    while (nsize > 0) {
        ...
        while (true) {
            int err = lfs_bd_prog(
                lfs, &file->cache, &lfs->rcache, true, file->block, file->off, data, diff);
            if (err) {
                if (err == LFS_ERR_CORRUPT) {
                    goto relocate;
                }
...
static int lfs_file_rawtruncate(lfs_t *lfs, lfs_file_t *file, lfs_off_t size)
{
    ...
    if (size < oldsize) {
        ...
        err = lfs_ctz_find(
            lfs, NULL, &file->cache, file->ctz.head, file->ctz.size, size, &file->block, &file->off);
        if (err) {
            return err;
        }
...
static lfs_ssize_t
lfs_rawgetattr(lfs_t *lfs, const char *path, uint8_t type, void *buffer, lfs_size_t size)
{
    lfs_mdir_t cwd;
    lfs_stag_t tag = lfs_dir_find(lfs, &cwd, &path, NULL);
    if (tag < 0) {
...
#ifndef LFS_READONLY
static int
lfs_rawsetattr(lfs_t *lfs, const char *path, uint8_t type, const void *buffer, lfs_size_t size)
{
...
static int lfs_fs_demove(lfs_t *lfs)
{
    ...
    lfs_fs_prepmove(lfs, 0x3ff, NULL);
    err =
        lfs_dir_commit(lfs, &movedir, LFS_MKATTRS({ LFS_MKTAG(LFS_TYPE_DELETE, moveid, 0), NULL }));
    if (err) {
        return err;
    }
    ...
    while (!lfs_pair_isnull(pdir.tail)) {
        ...
        if (!pdir.split) {
            ...
            if (tag == LFS_ERR_NOENT) {
                // we are an orphan
                LFS_DEBUG(
                    "Fixing orphan {0x%" PRIx32 ", 0x%" PRIx32 "}", pdir.tail[0], pdir.tail[1]);
            ...
            if (!lfs_pair_sync(pair, pdir.tail)) {
                ...
                err = lfs_dir_commit(
                    lfs, &pdir, LFS_MKATTRS({ LFS_MKTAG(LFS_TYPE_SOFTTAIL, 0x3ff, 8), pair }));
                lfs_pair_fromle32(pair);
                if (err) {
                    return err;
                }
        ...
        while (dir.off + sizeof(entry.d) <= (0x7fffffff & dir.d.size) - 4) {
            ...
            if ((0x70 & entry.d.type) == (0x70 & LFS1_TYPE_REG)) {
                err = lfs_ctz_traverse(
                    lfs, NULL, &lfs->rcache, entry.d.u.file.head, entry.d.u.file.size, cb, data);
                if (err) {
                    return err;
                }
            }
