...
int imx_salvo_phy_init(uintptr_t base)
{
    ...
    for (unsigned int i = 0; i < ARRAY_SIZE(cdns_nxp_sequence_pair); i++) {
        mmio_write_32(base + cdns_nxp_sequence_pair[i].reg_offset,
                             cdns_nxp_sequence_pair[i].val);
    }

    mmio_clrsetbits_32(base + TB_ADDR_TX_RCVDETSC_CTRL, 0, RXDET_IN_P3_32KHZ);
    mmio_clrsetbits_32(base + UTMI_REG15, TXVALID_GATE_THRESHOLD_HS_MASK,
                                          TXVALID_GATE_THRESHOLD_HS_0US);
