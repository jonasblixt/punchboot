...
static int dwc3_command(uint8_t ep, uint32_t cmd, uint32_t p0, uint32_t p1, uint32_t p2)
{
    ...
    */
    ...
    pb_write32((DWC3_DEPCMD_ACT | cmd), (base + DWC3_DEPCMD_0 + (0x10 * ep)));
...
static int dwc3_transfer_no_wait(int ep, void *bfr, size_t sz)
{
    ...
    LOG_INFO("trx EP%u %s, %p, sz %zu bytes", (ep >> 1), (ep & 1 ? "IN" : "OUT"), bfr, sz);
...
static int dwc3_transfer(int ep, void *bfr, size_t sz)
{
    ...
    while ((trb->control & 1) == 1) {
        ...
        if ((ep == USB_EP2_OUT) || (ep == USB_EP1_IN)) {
            dwc3_process();
        }
    }
...
static void dwc3_reset(void)
{
    ...
    dwc3_transfer_no_wait(USB_EP0_OUT, (uint8_t *)&setup_pkt, sizeof(struct usb_setup_packet));
}
...
int dwc3_init(__iomem base_addr)
{
    ...
    pb_setbit32((DWC3_PHY_CTRL1_RESET | DWC3_PHY_CTRL1_ATERESET), base + DWC3_PHY_CTRL1);

    pb_setbit32(DWC3_PHY_CTRL0_REF_SSP_EN, base + DWC3_PHY_CTRL0);

    pb_setbit32(DWC3_PHY_CTRL2_TXENABLEN0, base + DWC3_PHY_CTRL2);

    pb_clrbit32((DWC3_PHY_CTRL1_RESET | DWC3_PHY_CTRL1_ATERESET), base + DWC3_PHY_CTRL1);
...
int dwc3_set_configuration(void)
{
    ...
    pb_setbit32((1 << USB_EP2_OUT) | (1 << USB_EP1_IN), base + DWC3_DALEPENA);
