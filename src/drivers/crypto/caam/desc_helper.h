...
#define JUMP_LOCAL(cla, test, cond, offset)                                           \
    (CMD_JUMP_TYPE | CMD_CLASS(cla) | JUMP_TYPE(LOCAL) | JUMP_TST_TYPE(test) | cond | \
     JMP_LOCAL_OFFSET(offset))
/**
 * @brief  Jump Local of class 1 to descriptor offset \a offset
 *          if test \a test meet the condition \a cond
 */
#define JUMP_C1_LOCAL(test, cond, offset) JUMP_LOCAL(CLASS_1, test, cond, offset)
...
#define LD_IMM(cla, dst, len) \
    (CMD_LOAD_TYPE | CMD_CLASS(cla) | CMD_IMM | LOAD_DST(dst) | LOAD_LENGTH(len))
 ...
 *         class \a cla
 */
#define LD_NOIMM(cla, dst, len)  (CMD_LOAD_TYPE | CMD_CLASS(cla) | LOAD_DST(dst) | LOAD_LENGTH(len))
...
#define LD_NOIMM_OFF(cla, dst, len, off) \
    (CMD_LOAD_TYPE | CMD_CLASS(cla) | LOAD_DST(dst) | LOAD_OFFSET(off) | LOAD_LENGTH(len))
...
#define FIFO_LD(cla, dst, act, len)                                                       \
    (CMD_FIFO_LOAD_TYPE | CMD_CLASS(cla) | FIFO_LOAD_INPUT(dst) | FIFO_LOAD_ACTION(act) | \
...
#define FIFO_LD_SGT(cla, dst, act, len)                                     \
    (CMD_FIFO_LOAD_TYPE | CMD_CLASS(cla) | CMD_SGT | FIFO_LOAD_INPUT(dst) | \
...
#define FIFO_LD_EXT(cla, dst, act)                                                \
    (CMD_FIFO_LOAD_TYPE | FIFO_LOAD_EXT | CMD_CLASS(cla) | FIFO_LOAD_INPUT(dst) | \
...
#define FIFO_LD_IMM(cla, dst, act, len)                                     \
    (CMD_FIFO_LOAD_TYPE | CMD_IMM | CMD_CLASS(cla) | FIFO_LOAD_INPUT(dst) | \
     FIFO_LOAD_ACTION(act) | FIFO_LOAD_LENGTH(len))
...
#define ST_NOIMM_OFF(cla, src, len, off) \
    (CMD_STORE_TYPE | CMD_CLASS(cla) | STORE_SRC(src) | STORE_OFFSET(off) | STORE_LENGTH(len))

/**
 * @brief  FIFO Store from register \a src of length \a len
 */
#define FIFO_ST(src, len) (CMD_FIFO_STORE_TYPE | FIFO_STORE_OUTPUT(src) | FIFO_STORE_LENGTH(len))

/**
 * @brief  FIFO Store from register \a src. \n
 *         The length is externally defined
 */
#define FIFO_ST_EXT(src)  (CMD_FIFO_STORE_TYPE | FIFO_LOAD_EXT | FIFO_STORE_OUTPUT(src))
...
#define FIFO_ST_SGT(src, len) \
    (CMD_FIFO_STORE_TYPE | CMD_SGT | FIFO_STORE_OUTPUT(src) | FIFO_STORE_LENGTH(len))

/**
 * @brief  RNG State Handle instantation operation for \a sh id
 */
#define RNG_SH_INST(sh) \
    (CMD_OP_TYPE | OP_TYPE(CLASS1) | OP_ALGO(RNG) | ALGO_RNG_SH(sh) | ALGO_AS(RNG_INSTANTIATE))

/**
 * @brief  RNG Generates Secure Keys
 */
#define RNG_GEN_SECKEYS \
    (CMD_OP_TYPE | OP_TYPE(CLASS1) | OP_ALGO(RNG) | ALGO_RNG_SK | ALGO_AS(RNG_GENERATE))

/**
 * @brief  RNG Generates Data
 */
#define RNG_GEN_DATA      (CMD_OP_TYPE | OP_TYPE(CLASS1) | OP_ALGO(RNG) | ALGO_AS(RNG_GENERATE))

/**
 * @brief  HASH Init Operation of algorithm \a algo
 */
#define HASH_INIT(algo)   (CMD_OP_TYPE | OP_TYPE(CLASS2) | (algo) | ALGO_AS(INIT) | ALGO_ENCRYPT)

/**
 * @brief  HASH Update Operation of algorithm \a algo
 */
#define HASH_UPDATE(algo) (CMD_OP_TYPE | OP_TYPE(CLASS2) | (algo) | ALGO_AS(UPDATE) | ALGO_ENCRYPT)

/**
 * @brief  HASH Final Operation of algorithm \a algo
 */
#define HASH_FINAL(algo)  (CMD_OP_TYPE | OP_TYPE(CLASS2) | (algo) | ALGO_AS(FINAL) | ALGO_ENCRYPT)

/**
 * @brief  HASH Init and Final Operation of algorithm \a algo
 */
#define HASH_INITFINAL(algo) \
    (CMD_OP_TYPE | OP_TYPE(CLASS2) | (algo) | ALGO_AS(INIT_FINAL) | ALGO_ENCRYPT)

/**
 * @brief  HMAC Init Decryption Operation of algorithm \a algo
 */
#define HMAC_INIT_DECRYPT(algo) \
    (CMD_OP_TYPE | OP_TYPE(CLASS2) | (algo) | ALGO_AS(INIT) | ALGO_AAI(DIGEST_HMAC) | ALGO_DECRYPT)
...
#define HMAC_INIT_PRECOMP(algo)                                                               \
    (CMD_OP_TYPE | OP_TYPE(CLASS2) | (algo) | ALGO_AS(INIT) | ALGO_AAI(DIGEST_HMAC_PRECOMP) | \
     ALGO_ENCRYPT)
/**
 * @brief  HMAC Final Operation of algorithm \a algo with Precomp key
 */
#define HMAC_FINAL_PRECOMP(algo)                                                               \
    (CMD_OP_TYPE | OP_TYPE(CLASS2) | (algo) | ALGO_AS(FINAL) | ALGO_AAI(DIGEST_HMAC_PRECOMP) | \
     ALGO_ENCRYPT)
...
#define LD_KEY_PLAIN(cla, dst, len) \
    (CMD_KEY_TYPE | CMD_CLASS(cla) | KEY_PTS | KEY_DEST(dst) | KEY_LENGTH(len))

/**
 * @brief   Load a split key of length \a len.
 */
#define LD_KEY_SPLIT(len) \
    (CMD_KEY_TYPE | CMD_CLASS(CLASS_2) | KEY_DEST(MDHA_SPLIT) | KEY_LENGTH(len))
