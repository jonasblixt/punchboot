...
static int
virtio_xfer(struct virtq *q, bool read, uint32_t queue_index, uintptr_t buf, size_t length)
{
...
int virtio_serial_init(uintptr_t base_)
{
    ...
    LOG_INFO("Detected virtio serial @%" PRIxPTR ", features = 0x%x", base, features);

    mmio_write_32(base + VIRTIO_MMIO_STATUS, 0);
    mmio_clrsetbits_32(
        base + VIRTIO_MMIO_STATUS, 0, VIRTIO_STATUS_ACKNOWLEDGE | VIRTIO_STATUS_DRIVER);
    ...
    struct virtio_serial_config *cfg = (struct virtio_serial_config *)(base + VIRTIO_MMIO_CONFIG);
    ...
    return virtio_xfer(&ctrl_tx, false, 3, (uintptr_t)&ctrlm, sizeof(struct virtio_serial_control));
}
