...
static struct fuse fuse_uid0 = IMX8M_FUSE_BANK_WORD(0, 1, "UID0");

static struct fuse fuse_uid1 = IMX8M_FUSE_BANK_WORD(0, 2, "UID1");

static struct fuse rom_key_revoke_fuse = IMX8M_FUSE_BANK_WORD(9, 3, "Revoke");

static const char platform_namespace_uuid[] = "\x32\x92\xd7\xd2\x28\x25\x41\x00\x90\xc3\x96\x8f\x29"
                                              "\x60\xc9\xf2";
...
int plat_get_uuid(char *out)
{
    ...
    return uuid_gen_uuid3(platform_namespace_uuid, (const char *)uid, 8, out);
}
...
int plat_early_init(void)
{
    ...
    pb_write32((FRAC_PLL_CLKE_MASK | FRAC_PLL_REFCLK_SEL_OSC_25M | FRAC_PLL_LOCK_SEL_MASK |
                FRAC_PLL_NEWDIV_VAL_MASK | FRAC_PLL_REFCLK_DIV_VAL(4) | FRAC_PLL_OUTPUT_DIV_VAL(0) |
    ...
    while (((pb_read32(ARM_PLL_CFG0) & FRAC_PLL_LOCK_MASK) != (uint32_t)FRAC_PLL_LOCK_MASK))
        __asm__("nop");

    pb_clrbit32(FRAC_PLL_NEWDIV_VAL_MASK, ARM_PLL_CFG0);

    pb_setbit32(SSCG_PLL_CLKE_MASK | SSCG_PLL_DIV2_CLKE_MASK | SSCG_PLL_DIV3_CLKE_MASK |
                    SSCG_PLL_DIV4_CLKE_MASK | SSCG_PLL_DIV5_CLKE_MASK | SSCG_PLL_DIV6_CLKE_MASK |
                    SSCG_PLL_DIV8_CLKE_MASK | SSCG_PLL_DIV10_CLKE_MASK | SSCG_PLL_DIV20_CLKE_MASK,
                SYS_PLL1_CFG0);
    ...
    ocotp_init(CONFIG_IMX_OCOTP_BASE, CONFIG_IMX_OCOTP_WORDS_PER_BANK);
...
int plat_mmu_init(void)
{
    ...
    size_t ro_size = ((uintptr_t)&_ro_data_region_end) - ((uintptr_t)&_ro_data_region_start);

    uintptr_t code_start = (uintptr_t)&_code_start;
    size_t code_size = ((uintptr_t)&_code_end) - ((uintptr_t)&_code_start);
    ...
    size_t rw_size = ((uintptr_t)&_data_region_end) - ((uintptr_t)&_data_region_start);

    uintptr_t bss_start = (uintptr_t)&_zero_region_start;
    size_t bss_size = ((uintptr_t)&_zero_region_end) - ((uintptr_t)&_zero_region_start);

    uintptr_t bb_start = (uintptr_t)&_no_init_start;
    size_t bb_size = ((uintptr_t)&_no_init_end) - ((uintptr_t)&_no_init_start);

    reset_xlat_tables();

    mmap_add_region(code_start, code_start, code_size, MT_RO | MT_MEMORY | MT_EXECUTE);
    mmap_add_region(stack_start, stack_start, stack_size, MT_RW | MT_MEMORY | MT_EXECUTE_NEVER);
    mmap_add_region(ro_start, ro_start, ro_size, MT_RO | MT_MEMORY | MT_EXECUTE_NEVER);
    mmap_add_region(rw_start, rw_start, rw_size, MT_RW | MT_MEMORY | MT_EXECUTE_NEVER);
    mmap_add_region(bss_start, bss_start, bss_size, MT_RW | MT_MEMORY | MT_EXECUTE_NEVER);
    mmap_add_region(bb_start, bb_start, bb_size, MT_RW | MT_MEMORY | MT_EXECUTE_NEVER);

    /* Add ram */

    mmap_add_region(bb_start + bb_size,
                    bb_start + bb_size,
                    (1024 * 1024 * 1024),
...
int plat_console_init(void)
{
    ...
    return imx_uart_init(CONFIG_IMX_UART_BASE, CONFIG_IMX_UART_BAUDRATE);
}
...
int plat_pk_verify(uint8_t *signature,
...
int plat_status(void *response_bfr, size_t *response_size)
{
...
int plat_command(uint32_t command, void *bfr, size_t size, void *response_bfr, size_t *response_size)
{
    return board_command(&private, command, bfr, size, response_bfr, response_size);
}
