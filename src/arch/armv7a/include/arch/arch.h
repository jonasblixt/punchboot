...
#define MPID_MASK                                              \
    (MPIDR_MT_MASK | (MPIDR_AFFLVL_MASK << MPIDR_AFF2_SHIFT) | \
     (MPIDR_AFFLVL_MASK << MPIDR_AFF1_SHIFT) | (MPIDR_AFFLVL_MASK << MPIDR_AFF0_SHIFT))
...
#define SCTLR_RESET_VAL   (SCTLR_RES1 | SCTLR_NTWE_BIT | SCTLR_NTWI_BIT | SCTLR_CP15BEN_BIT)
...
/* HSCTLR definitions */
#define HSCTLR_RES1                                                                            \
    ((U(1) << 29) | (U(1) << 28) | (U(1) << 23) | (U(1) << 22) | (U(1) << 18) | (U(1) << 16) | \
     (U(1) << 11) | (U(1) << 4) | (U(1) << 3))
...
#define CPACR_ENABLE_FP_ACCESS    ((U(0x3) << CPACR_CP11_SHIFT) | (U(0x3) << CPACR_CP10_SHIFT))
...
#define SPSR_MODE32(mode, isa, endian, aif)                                             \
    (MODE_RW_32 << MODE_RW_SHIFT | ((mode) & MODE32_MASK) << MODE32_SHIFT |             \
     ((isa) & SPSR_T_MASK) << SPSR_T_SHIFT | ((endian) & SPSR_E_MASK) << SPSR_E_SHIFT | \
...
/* GICv3 CPU Interface system register defines. The format is:
 *  coproc, opt1, CRn, CRm, opt2 */
...
/* 64 bit GICv3 CPU Interface system register defines. The format is:
 * coproc, opt1, CRm */
...
void arch_jump(void *addr, void *p0, void *p1, void *p2, void *p3) __attribute__((noreturn));
